...
function GridDeploy({
  ...
  projectList: ParsedDeploymentDirStruct[] | null;
  showMultiSelect?: boolean;
  showFilter: boolean;
  onGridLocalProjectListChange: (state: ParsedDeploymentDirStruct[]) => void;
  onGridSelectedProjectListChange: (state: ParsedDeploymentDirStruct[]) => void;
  localProjectList: ParsedDeploymentDirStruct[];
  selectedGridItems: ParsedDeploymentDirStruct[];
}) {
  // all data
  ...
  useEffect(() => {
    if (projectList) {
      ...
      onGridSelectedProjectListChange([] as ParsedDeploymentDirStruct[]);
  ...
  const handleIsSelectedChange = (
    ...
    row_data: ParsedDeploymentDirStruct
  ) => {
    ...
    if (event.target.checked === true) {
      ...
      currentSelectedGridItems = currentSelectedGridItems.filter(
        (el) => el.project_name !== row_data.project_name
      );
      onGridSelectedProjectListChange(currentSelectedGridItems);
    }
  };
  ...
  const getRowSelectedStatus = (row: ParsedDeploymentDirStruct) => {
    const data = selectedGridItems.find(
      (el) => el.project_name === row.project_name
    );
    return data != null;
  };
      ...
      <table className="table table-pin-rows">
        ...
        <tbody>
          {localProjectList?.map((row) => {
            ...
            return (
              <tr key={row.project_name} className="hover">
                {showMultiSelect && (
